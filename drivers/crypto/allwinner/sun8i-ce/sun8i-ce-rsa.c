...
#define RSA_LENDIV 4
	...
	/*modulus = caam_read_raw_data(ctx->raw_key.n, &blk_size);
	if (!modulus) {
	...
	case 4096:
		...
		break;
	default:
		dev_err(ss->dev, "RSA invalid keysize %u\n", blk_size * 8);
		return -EINVAL;
		/* TODO fallback ? */
	}
...
int sun8i_rsa_sign(struct akcipher_request *req)
{
	...
	dev_err(ctx->ss->dev, "%s un-implemented\n", __func__);
return -EFAULT;
}
			  ...
			  unsigned int keylen)
{
	...
	dev_info(ctx->ss->dev, "%s keylen=%u loaded n_sz=%u\n", __func__, keylen, ctx->raw_key.n_sz);
	return 0;
}
