...
static int sun8i_rsa_operation(struct akcipher_request *req, int dir)
{
	...
	dev_dbg(ctx->ss->dev, "%s modulus %zu e=%zu d=%zu c=%zu slen=%u dlen=%u dir=%d sg=%d/%d\n",
		__func__,
		ctx->raw_key.n_sz, ctx->raw_key.e_sz, ctx->raw_key.d_sz,
		ctx->raw_key.n_sz,
		req->src_len, req->dst_len, dir, sg_nents(req->src), sg_nents(req->dst));

	if (sg_nents(req->src) > 8 || sg_nents(req->dst) > 8)
		need_fallback = true;
...
#define RSA_LENDIV 4
	...
	memcpy(modulus, ctx->modulus, ctx->raw_key.n_sz);
	...
	case 4096:
		...
		break;
	default:
		dev_err(ss->dev, "RSA invalid keysize %zu\n", blk_size * 8);
		return -EINVAL;
		/* TODO fallback ? */
	}
	...
	if (ctx->raw_key.e_sz == 256)
		need_fallback = true;

	/* check if fallback is necessary */
	if (req->src_len != blk_size || req->src_len % 4 != 0 ||
	...
	if (dir == CE_ENCRYPTION) {
		...
		memcpy(key, ctx->raw_key.d, ctx->raw_key.d_sz);
	...
	print_hex_dump(KERN_INFO, "KEY ", DUMP_PREFIX_NONE, 16, 1,
		       ctx->raw_key.e, ctx->raw_key.e_sz, false);
...
int sun8i_rsa_decrypt(struct akcipher_request *req)
{
	...
	dev_dbg(ctx->ss->dev, "%s modulus %zu e=%zu d=%zu c=%zu slen=%u dlen=%u\n",
		__func__,
		ctx->raw_key.n_sz, ctx->raw_key.e_sz, ctx->raw_key.d_sz,
		ctx->raw_key.n_sz,
		req->src_len, req->dst_len);
