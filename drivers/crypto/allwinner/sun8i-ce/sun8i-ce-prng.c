...
{
	struct sun8i_prng_tfm_ctx *ctx = crypto_rng_ctx(tfm);
	struct ce_task *cet;
	int flow, ret = 0;
	void *data;
	size_t len;
	...
	cet->t_id = flow;
	cet->t_common_ctl = ctx->ss->variant->prng | CE_COMM_INT;
	cet->t_dlen = PRNG_DATA_SIZE / 4;
	ss->chanlist[flow].op_mode = 0;
	ss->chanlist[flow].op_dir = 0;
	ss->chanlist[flow].method = ctx->ss->variant->prng;
...
rebegin:
	...
	dev_dbg(ss->dev, "%s Rebegin %u dlen=%u steplen=%zu\n", __func__, slen, dlen, len);
...
{
	struct sun8i_prng_tfm_ctx *ctx = crypto_tfm_ctx(tfm);
	struct sun8i_ss_alg_template *algt;
