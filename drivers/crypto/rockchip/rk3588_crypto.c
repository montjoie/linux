...
static struct rk_crypto_template rk_cipher_algs[] = {
	{
		...
		.alg.skcipher = {
			...
			.encrypt		= rk_skcipher_encrypt,
			.decrypt		= rk_skcipher_decrypt,
		}
	},
	{
		.type = CRYPTO_ALG_TYPE_SKCIPHER,
		.rk_mode = RK_CRYPTO_AES_CBC,
		.alg.skcipher = {
...
#ifdef CONFIG_CRYPTO_DEV_ROCKCHIP2_RSA
	{
		.type = CRYPTO_ALG_TYPE_AKCIPHER,
		.alg.rsa = {
			.encrypt = rk_rsa_encrypt,
			.decrypt = rk_rsa_decrypt,
			.sign = rk_rsa_sign,
			.verify = rk_rsa_verify,
			.set_priv_key = rk_rsa_set_priv_key,
			.set_pub_key = rk_rsa_set_pub_key,
			.max_size = rk_rsa_max_size,
			.init = rk_rsa_init,
			.exit = rk_rsa_exit,
			.base = {
				.cra_name = "rsa",
				.cra_driver_name = "rsa-rk-ce",
				.cra_priority = 400,
				.cra_flags = CRYPTO_ALG_TYPE_AKCIPHER |
					CRYPTO_ALG_ASYNC | CRYPTO_ALG_NEED_FALLBACK,
				.cra_ctxsize = sizeof(struct rk_rsa_tfm_ctx),
				.cra_module = THIS_MODULE,
				.cra_alignmask = 3,
			}
		}
...
static int rk_crypto_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(rkc->rst)) {
		err = PTR_ERR(rkc->rst);
		dev_err(&pdev->dev, "Fail to get resets\n");
		goto err_crypto;
	}
...
#ifdef CONFIG_CRYPTO_DEV_ROCKCHIP2_VERBOSE
	v = readl(rkc->reg + RK_CRYPTO_CLK_CTL);
	dev_info(&pdev->dev, "CRYPTO_CLK_CTL %x\n", v);
	v = readl(rkc->reg + RK_CRYPTO_RST_CTL);
	dev_info(&pdev->dev, "CRYPTO_RST_CTL %x\n", v);

	v = readl(rkc->reg + CRYPTO_AES_VERSION);
	dev_info(&pdev->dev, "CRYPTO_AES_VERSION %x\n", v);
	if (v & BIT(17))
		dev_info(&pdev->dev, "AES 192\n");

	v = readl(rkc->reg + CRYPTO_DES_VERSION);
	dev_info(&pdev->dev, "CRYPTO_DES_VERSION %x\n", v);
	v = readl(rkc->reg + CRYPTO_SM4_VERSION);
	dev_info(&pdev->dev, "CRYPTO_SM4_VERSION %x\n", v);
	v = readl(rkc->reg + CRYPTO_HASH_VERSION);
	dev_info(&pdev->dev, "CRYPTO_HASH_VERSION %x\n", v);
	v = readl(rkc->reg + CRYPTO_HMAC_VERSION);
	dev_info(&pdev->dev, "CRYPTO_HMAC_VERSION %x\n", v);
	v = readl(rkc->reg + CRYPTO_RNG_VERSION);
	dev_info(&pdev->dev, "CRYPTO_RNG_VERSION %x\n", v);
	v = readl(rkc->reg + CRYPTO_PKA_VERSION);
	dev_info(&pdev->dev, "CRYPTO_PKA_VERSION %x\n", v);
	v = readl(rkc->reg + CRYPTO_CRYPTO_VERSION);
	dev_info(&pdev->dev, "CRYPTO_CRYPTO_VERSION %x\n", v);
#endif
...
{
	struct rk_crypto_dev *rkc = platform_get_drvdata(pdev);
	struct rk_crypto_dev *first;
