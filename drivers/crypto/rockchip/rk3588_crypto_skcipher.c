...
static void rk_print(struct rk_crypto_dev *rkc)
{
	u32 v;

	v = readl(rkc->reg + 0x0018);
	...
	v = readl(rkc->reg + 0x0020);
	dev_info(rkc->dev, "DMA_LLI_ADDR %x\n", v);
	v = readl(rkc->reg + 0x0024);
	dev_info(rkc->dev, "DMA_LLI_RADDR %x\n", v);
	v = readl(rkc->reg + 0x0028);
	dev_info(rkc->dev, "DMA_LLI_WADDR %x\n", v);
	v = readl(rkc->reg + 0x002C);
	dev_info(rkc->dev, "DMA_LLI_ITEMID %x\n", v);

	v = readl(rkc->reg + 0x004C);
	...
	case 1:
		...
		break;
	case 2:
		dev_info(rkc->dev, "serial NULK state\n");
		break;
	default:
		dev_info(rkc->dev, "serial invalid state\n");
		break;
	}
	switch (v & 0xC) {
	case 0:
		dev_info(rkc->dev, "mac IDLE state\n");
		break;
	case 1:
		dev_info(rkc->dev, "macl PRE state\n");
		break;
	default:
		dev_info(rkc->dev, "mac TODO state\n");
		break;
	}
	switch (v & 0x30) {
	case 0:
		dev_info(rkc->dev, "parallel IDLE state\n");
		break;
	case 1:
		dev_info(rkc->dev, "parallel PRE state\n");
		break;
	default:
		dev_info(rkc->dev, "parallel TODO state\n");
		break;
	}
	switch (v & 0xC0) {
	case 0:
		dev_info(rkc->dev, "ccm IDLE state\n");
		break;
	case 1:
		dev_info(rkc->dev, "ccm PRE state\n");
		break;
	default:
		dev_info(rkc->dev, "ccm TODO state\n");
		break;
	}
	switch (v & 0xF00) {
	case 0:
		dev_info(rkc->dev, "gcm IDLE state\n");
		break;
	case 1:
		dev_info(rkc->dev, "gcm PRE state\n");
		break;
	case 2:
		dev_info(rkc->dev, "gcm NA state\n");
		break;
	case 3:
		dev_info(rkc->dev, "gcm PC state\n");
		break;
	}
	switch (v & 0xC00) {
	case 0:
		dev_info(rkc->dev, "hash IDLE state\n");
		break;
	case 1:
		dev_info(rkc->dev, "hash IPAD state\n");
		break;
	case 2:
		dev_info(rkc->dev, "hash TEXT state\n");
		break;
	case 4:
		dev_info(rkc->dev, "hash OPAD state\n");
		break;
	case 8:
		dev_info(rkc->dev, "hash OPAD EXT state\n");
		break;
	default:
		dev_info(rkc->dev, "hash invalid state\n");
		break;
	}

	v = readl(rkc->reg + RK_CRYPTO_DMA_INT_ST);
	dev_info(rkc->dev, "RK_CRYPTO_DMA_INT_ST %x\n", v);
}
...
int rk_aes_xts_setkey(struct crypto_skcipher *cipher, const u8 *key,
		  unsigned int keylen)
{
...
int rk_skcipher_encrypt(struct skcipher_request *req)
{
	struct rk_cipher_rctx *rctx = skcipher_request_ctx(req);
	struct crypto_skcipher *tfm = crypto_skcipher_reqtfm(req);
	struct skcipher_alg *alg = crypto_skcipher_alg(tfm);
...
int rk_skcipher_decrypt(struct skcipher_request *req)
{
	struct rk_cipher_rctx *rctx = skcipher_request_ctx(req);
	struct crypto_skcipher *tfm = crypto_skcipher_reqtfm(req);
	struct skcipher_alg *alg = crypto_skcipher_alg(tfm);
	...
	rctx->mode = algt->rk_mode | RK_CRYPTO_DEC;
	return rk_cipher_handle_req(req);
}
	...
	while (sgs && sgd && len) {
		...
		if (algt->is_xts) {
			...
			for (i = 0; i < (ctx->keylen / 8); i++) {
				...
				writel(v, rkc->reg + RK_CRYPTO_CH4_KEY0 + i * 4);
			}
		} else {
		...
		/*writel(0x00030000, rkc->reg + RK_CRYPTO_FIFO_CTL);*/
