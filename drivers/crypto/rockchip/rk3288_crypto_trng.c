...
static int rk3288_trng_read(struct hwrng *hwrng, void *data, size_t max, bool wait)
{
...
#ifdef CRYPTO_DEV_ROCKCHIP_DEBUG
	rk->hwrng_stat_req++;
	...
	if (count++ % 1000 == 0)
		pr_info("%s sample=%u\n", __func__, rk->rng_sample);

#define HIWORD_UPDATE(val, mask, shift) \
			((val) << (shift) | (mask) << ((shift) + 16))
	if (rk->rng_sample <= 0 || rk->rng_sample > 65535)
		rk->rng_sample = 500;
	v = RK_CRYPTO_OSC_ENABLE | rk->rng_sample;
...
/*	if (count++ < 30)
		print_hex_dump(KERN_INFO, "rng:", DUMP_PREFIX_NONE, 16, 4, data, todo, false);*/
	err = todo;
