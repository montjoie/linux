...
static void ch348_process_status_urb(struct urb *urb)
{
	...
	for (i = 0; i < urb->actual_length;) {
		...
		status_len = 3;
		/*
		dev_info(&port->dev,"status with reg_iir 0x%02x for port %d\n",
				 status_entry->reg_iir, status_entry->portnum);*/
		...
		if (!status_entry->reg_iir) {
			...
			complete(&ch348->ports[status_entry->portnum].complete);
		} else {
			dev_warn(&port->dev,
		       ...
		       const unsigned char *buf, int count)
{
	...
	reinit_completion(&ch348_port->complete);
	ret = usb_serial_generic_write(tty, port, buf, min(count, max_tx_size));
	/*dev_info(&port->dev, "%s count=%d max_tx_size=%d ret=%d\n", __func__, count, max_tx_size, ret);*/
	if (ret <= 0)
		return ret;
	msleep(1);
/*	usleep_range(200, 200);*/

	if (!wait_for_completion_interruptible_timeout(&ch348_port->complete,
...
static int ch348_attach(struct usb_serial *serial)
{
	...
	for (i = 0; i < CH348_MAXPORT; i++)
		init_completion(&ch348->ports[i].complete);
